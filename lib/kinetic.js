...
export class PDU extends stream.Readable {
    ...
    _parse(data) {
        ...
        try {
            const protobuf = data.slice(HEADER_SZ, HEADER_SZ + protobufSize);
...
export class InitPDU extends PDU {
    constructor(getLog, options = {}) {
        ...
        const message = new protoBuild.Command({
            header: {
                connectionID,
        ...
        this.setProtobuf(new protoBuild.Message({
            authType: 'UNSOLICITEDSTATUS',
            commandBytes: message.toBuffer(),
        }));
        return this;
    }
}
