...
const newClusterVErsion = new Buffer('2');
let timer = '';
const requestNumber = 100000;

time0 = process.hrtime();
for (let i = 0; i < requestNumber; i++) {
    a = new kinetic.SetClusterVersionPDU(
        sequence, newClusterVErsion, clusterversion).read();
...
const wstream = fs.createWriteStream(requestNumber + 'setClusterVersionJSON');
wstream.write(timer);
wstream.end();
