...
time0 = process.hrtime();
for (let i = 0; i < requestNumber; i++) {
    b = new kinetic.NoOpResponsePDU(sequence, sequence, detailedMessage).read();
...
timer += 'size of the noopPDU response : ' + pdb.getProtobufSize() + ' bytes' +
    os.EOL;

const wstream = fs.createWriteStream(requestNumber + 'noop');
wstream.write(timer);
wstream.end();
